...
namespace LoggingKata
{
    class Program
    {
        ...
        {
            // TODO:  Find the two Taco Bells that are the furthest from one another.  HINT:  You'll need two nested forloops ---------------------------
            logger.LogInfo("Log initialized");

                                                                    // use File.ReadAllLines(path) to grab all the lines from your csv file
                                                                    // Log and error if you get 0 lines and a warning if you get 1 line
            var lines = File.ReadAllLines(csvPath);                 // lines is the array of lines in csv,  so each line in the CSV is an element of lines , this gets all lines in CSV file
           ...
           var parser = new TacoParser();                           // Create a new instance of your TacoParser class
            ...
            var locations = lines.Select(parser.Parse).ToArray();   // this is what gets each individual line to put thru the lines Parser, select reiterates thru each line
                                                                    // DON'T FORGET TO LOG YOUR STEPS   // Now that your Parse method is completed, START BELOW ----------
                                                     
            ITrackable location1 = null;                // TODO: Create two `ITrackable` variables with initial values of `null`. These will be used to store your two taco bells that are the farthest from each other.
            ITrackable location2 = null;

            double distance = 0.0;                                  // Create a `double` variable to store the distance
                                                                    // Include the Geolocation toolbox, so you can compare locations: `using GeoCoordinatePortable;`  (using system)
                                                                    //HINT NESTED LOOPS SECTION-------------// Do a loop for your locations to grab each location as the origin (perhaps: `locA`) (line 31)
